...
#pragma hdrstop
// ---------------------------------------------------------------------------
#include "ctlSelfElevation.h"
#include <Strsafe.h>
// ---------------------------------------------------------------------------
#pragma package(smart_init)
// ---------------------------------------------------------------------------

TctlSelfElevation::TctlSelfElevation()
{
	...
	// try
	// {
	// lblIsUserInAdminGroup->Caption = (IsUserInAdminGroup()) ? L"True" : L"False";
	// }
	// catch (DWORD dwError)
	// {
	// lblIsUserInAdminGroup->Caption = L"N/A";
	...
	// try
	// {
	// lblIsRunAsAdmin->Caption = IsRunAsAdmin() ? L"True" : L"False";
	// }
	// catch (DWORD dwError)
	// {
	// lblIsRunAsAdmin->Caption = L"N/A";
	...
	// try
	// {
	// bool const isElevated = IsProcessElevated();
	// // Get and display the process elevation information.
	// lblIsProcessElevated->Caption = isElevated ? L"True" : L"False";
	//
	...
	//
	// Button_SetElevationRequiredState(cmdSelfElevate->Handle, !isElevated);
	// }
	// catch (DWORD dwError)
	// {
	// lblIsProcessElevated->Caption = L"N/A";
	...
	// lblIntegrityLevel->Caption = L"Untrusted";
	// break;
	// case SECURITY_MANDATORY_LOW_RID:
	// lblIntegrityLevel->Caption = L"Low";
	// break;
	// case SECURITY_MANDATORY_MEDIUM_RID:
	// lblIntegrityLevel->Caption = L"Medium";
	// break;
	// case SECURITY_MANDATORY_HIGH_RID:
	// lblIntegrityLevel->Caption = L"High";
	// break;
	// case SECURITY_MANDATORY_SYSTEM_RID:
	// lblIntegrityLevel->Caption = L"System";
	// break;
	// default:
	// lblIntegrityLevel->Caption = L"Unknown";
	// break;
	// }
	// }
	// catch (DWORD dwError)
	// {
	// lblIntegrityLevel->Caption = L"N/A";
...
// }
//
bool TctlSelfElevation::IsUserInAdminGroup()
{
	int           fInAdminGroup = false;
...
// }
//
bool TctlSelfElevation::IsRunAsAdmin()
{
	int   fIsRunAsAdmin        = false;
	DWORD dwError              = ERROR_SUCCESS;
...
// }
//
bool TctlSelfElevation::IsProcessElevated()
{
...
// }
//
DWORD TctlSelfElevation::GetProcessIntegrityLevel()
{
	...
	// from GetTokenInformation because we have given it a nullptr buffer. On
	// exit cbTokenIL will tell the size of the integrity level information.
	if (!GetTokenInformation(hToken, TokenIntegrityLevel, nullptr, 0, &cbTokenIL))
	{
...
//
void TctlSelfElevation::ReportError(LPCWSTR pszFunction, DWORD dwError)
{
	wchar_t szMessage[200];
	if (SUCCEEDED(StringCchPrintf(szMessage, ARRAYSIZE(szMessage),
				L"%s failed w/err 0x%08lx", pszFunction, dwError)))
	{
		MessageBox(nullptr, szMessage, L"Error", MB_ICONERROR);
	}
}

void TctlSelfElevation::SelfElevateClick(HWND Handle)
{
	...
	if (!fIsRunAsAdmin)
	{
		...
		if (GetModuleFileName(nullptr, szPath, ARRAYSIZE(szPath)))
		{
			...
			{
				SendMessage(Handle, WM_CLOSE, 0, 0); // Quit itself
			}
		}
	}
	else
	{
		MessageBox(Handle, L"The process is running as administrator", L"UAC", MB_OK);
	}
